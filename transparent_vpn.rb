...
if ENV.include? 'script_type' then
	overwrite_ip = ENV['CFG_LOCAL_IP'] != ENV['ifconfig_local']
	               and ENV['CFG_LOCAL_IP'] != "default"
	...
	iptables_args = ["POSTROUTING", "-t", "nat", "-s", ENV['CFG_LOCAL_IP'], "-o",
	                 device, "-j", "SNAT", "--to-source", ENV['ifconfig_local']]
	...
	opts = OptionParser.new do |opts|
		...
		opts.on("-p", "--port [PORT]", Integer,
		        "The port for socat to listen on") do |port|
